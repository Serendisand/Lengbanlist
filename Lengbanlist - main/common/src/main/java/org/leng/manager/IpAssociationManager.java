...
public class IpAssociationManager {
    ...
    public IpAssociationManager(LengbanlistPlatform plugin) {
        this.plugin = plugin;
    }

    public void recordLogin(String playerName, String ip) {
        if (!isRealIp(ip)) return;
        plugin.getDatabaseManager().recordPlayerIp(playerName, ip, System.currentTimeMillis());
    }
    ...
    public boolean hasSuspiciousLogin(String playerName, String ip) {
    ...
    public List<String> getSuspiciousLoginDetails(String playerName, String ip) {
